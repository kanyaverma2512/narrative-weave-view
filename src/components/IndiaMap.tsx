...
export function IndiaMap({
  ...
  function fillFor(posts: number | undefined) {
    ...
    if (t > 0.7) return "color-mix(in oklch, var(--primary) 78%, white)";
    if (t > 0.45) return "color-mix(in oklch, var(--primary) 52%, white)";
  ...
  return (
    <div className={cn("relative overflow-hidden rounded-3xl bg-gradient-to-br from-sky/40 via-white to-mint/40 p-2", className)}>
      <svg viewBox={INDIA_VIEWBOX} className="h-full w-full" role="img" aria-label="Interactive map of India showing conversation activity by state">
        ...
        {cities.map((c) => (
          <g key={c.name} className="pointer-events-none">
            ...
            <circle cx={c.p[0]} cy={c.p[1]} r={Math.max(3.2, (c.posts / maxCity) * 7)} fill="var(--primary)" stroke="white" strokeWidth="1.6" />
        ...
        {hover && (
          <g className="pointer-events-none">
            <rect x={hover.x - 74} y={hover.y - 46} width="148" height="40" rx="12" fill="white" opacity="0.96" />
            <text x={hover.x} y={hover.y - 30} textAnchor="middle" fontSize="13" fontWeight="800" fill="var(--ink, #2b2340)">
              {hover.name}
            </text>
            <text x={hover.x} y={hover.y - 14} textAnchor="middle" fontSize="11" fill="#6b6480">
      ...
      <div className="absolute bottom-3 right-3 flex items-center gap-2 rounded-2xl bg-white/90 px-3 py-2 text-[10px] font-bold shadow-sm">
